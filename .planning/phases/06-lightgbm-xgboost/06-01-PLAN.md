...
**Function 8: `_train_and_evaluate_model(model_name, model, X_train, y_train, w_train, X_val, y_val, w_val, X_test, y_test, w_test, val_df, test_df, feature_names, study, root) -> dict`**
...
9. Save predictions (val + test combined):
   ...
   pred_path = root / "data" / "processed" / f"predictions_{model_name[:4] if model_name == 'lightgbm' else 'xgb'}.parquet"
   combined.write_parquet(pred_path)
   print(f"Predictions saved: {combined.height:,} rows to {pred_path}")
   ```
   NOTE: Use `predictions_lgbm.parquet` for lightgbm, `predictions_xgb.parquet` for xgboost. The abbreviation logic: if model_name == "lightgbm" -> "lgbm", if model_name == "xgboost" -> "xgb". Implement this cleanly:
   ```python
...
11. Generate PR curve:
    ...
    ```
12. Determine best_iteration: for lightgbm use `model.best_iteration_`, for xgboost use `model.best_iteration`.
13. Build and return the model entry dict (for model_results.json):
    ...
    entry = {
        "metadata": {
            ...
            "scale_pos_weight": round(float(spw), 4),  # spw must be passed or computed
...
7. **Evaluate LightGBM:**
   Call `_train_and_evaluate_model("lightgbm", best_lgbm, ...)` to get the model entry dict. Pass `lgbm_study` as the study argument.
...
10. **Evaluate XGBoost:**
    Call `_train_and_evaluate_model("xgboost", best_xgb, ...)` to get the model entry dict. Pass `xgb_study` as the study argument.
...
**IMPORTANT: Pass `spw` to `_train_and_evaluate_model` so it can be included in metadata.** Either pass it as a parameter or have the function accept it. Make sure the function signature includes everything needed.

**IMPORTANT: The `_round_dict` helper should be defined at module level or inside `_train_and_evaluate_model`.** Reuse it consistently for all float values in JSON output.
  </action>
  <verify>
Run the pipeline:
```bash
