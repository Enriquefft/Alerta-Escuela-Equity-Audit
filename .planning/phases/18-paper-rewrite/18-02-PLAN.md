...
2. Extracts calibration_by_decile.lightgbm_calibrated.deciles — a list of dicts with
   decile, mean_predicted, mean_observed, lift fields
   (IMPORTANT: field names are mean_predicted and mean_observed, NOT pred_mean/obs_rate)
3. Creates a grouped bar chart: x-axis = decile 1-10, two bars per decile:
   predicted probability (mean_predicted) and observed rate (mean_observed)
...
RF & 0.261 & 0.237 & 0.631 & {---} & {$<$0} \\
MLP & 0.238 & 0.210 & 0.607 & {---} & 0.012 \\
\bottomrule
\end{tabular}
```
Note: All values are confirmed from data/exports/model_results.json and data/exports/predictive_validity.json.
MLP BSS=0.012 is POSITIVE (not negative) — MLP uses sigmoid output without scale_pos_weight, so raw probabilities are valid.
Only LR, XGBoost, and RF have negative Brier Skill Scores.
...
The Brier Skill Score of 0.040 for the calibrated model is positive, reflecting good probability
calibration. Among the uncalibrated models, LR, XGBoost, and RF have negative Brier Skill Scores
due to scale_pos_weight distorting raw probabilities. MLP achieves a marginal positive BSS of
0.012 because its sigmoid output is not affected by scale_pos_weight; nevertheless, Platt scaling
on LightGBM remains the only well-calibrated model we recommend for probability-based decisions.
