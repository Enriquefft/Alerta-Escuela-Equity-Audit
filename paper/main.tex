...
\begin{abstract}
Nationally representative household surveys---available in most countries---enable proxy audits of deployed early warning systems (EWS) without requiring access to model code or training data. We apply this framework to Peru's Alerta Escuela, constructing a proxy dropout prediction model from publicly available ENAHO survey data (2018--2023, $N=150{,}135$) targeting the same school-age population. We have not accessed Alerta Escuela's predictions, training data, or operational feature set; our findings characterize disparities in survey-derived dropout risk modeling, not the deployed system itself. We train five model families (logistic regression, LightGBM, XGBoost, random forest, and MLP) and audit across language, geography, poverty, and sex dimensions. The calibrated LightGBM model (test PR-AUC = 0.236, top-decile lift = 2.54$\times$) exhibits a false negative rate (FNR) of 63.3\% for Spanish-speaking students [95\% CI: 0.608, 0.656] but only 21.6\% for indigenous-language speakers [0.137, 0.310]: the majority of Spanish-speaking dropouts are missed while indigenous students are over-flagged. This FNR rank order holds across all five model families. SHAP analysis shows the model predicts through spatial-structural features rather than identity features directly. Intersectional analysis identifies urban indigenous students as a potentially high-FNR subgroup (pooled estimate 0.69, $n=167$), though small sample size limits precision (95\% CI [0.39, 0.98]). A power analysis shows that confirming whether FNR exceeds 0.50 for this group requires approximately 8 ENAHO years, a methodological ceiling that argues for opening administrative data. The code and data pipeline are available at an anonymized version of the GitHub repository \url{https://anonymous.4open.science/r/Alerta-Escuela-Equity-Audit-42FC/}. \\
...
Educational early warning systems (EWS) are expanding across Latin America. Peru's Alerta Escuela, operated by the Ministry of Education (MINEDU), flags students at risk of leaving school using administrative data from the SIAGIE system \cite{minedu2023alerta}. These systems can enable earlier intervention, but whether they flag students equitably is almost entirely unknown. Predictive models can systematically disadvantage marginalized groups by encoding structural inequities into automated decisions affecting millions of students \cite{barocas2016big,mitchell2021algorithmic}.

Few studies audit deployed educational prediction systems in developing countries, even as the algorithmic fairness literature grows. Most fairness work focuses on US and European contexts, examines race and gender as primary dimensions, and does not incorporate survey weights or intersectional analysis \cite{kizilcec2020algorithmic,baker2022algorithmic,gardner2024debiasing}. In countries like Peru, the axes of disadvantage---mother tongue, geography, poverty---differ from those studied in the Global North.

About 16\% of Peru's population speaks an indigenous language, and only 37\% of indigenous students attend bilingual schools \cite{cueto2009explaining,cueto2016education}. Because SIAGIE administrative data is not publicly accessible, we construct a proxy dropout prediction model using ENAHO survey data \cite{inei2023enaho} spanning 150,135 student-year observations across 2018--2023.
...
We find a surveillance--invisibility axis: indigenous-language students are over-flagged while most Spanish-speaking dropouts are missed. SHAP analysis shows the model predicts through spatial proxy features, and the pattern holds across all five model families.

Our contributions are:
\begin{itemize}
  \item A proxy equity audit framework using only publicly available survey data, replicable in any country with nationally representative household surveys. The full pipeline is open-source.
  \item A survey-weighted fairness audit across seven demographic dimensions and three intersections, showing that indigenous-language students are over-flagged while most Spanish-speaking dropouts are missed---a pattern invisible to single-axis analysis \cite{crenshaw1989demarginalizing,buolamwini2018gender}.
  \item Evidence that these disparities arise from spatial proxy features, not protected attributes.
\end{itemize}
...
The features available in ENAHO differ substantially from SIAGIE: our proxy model lacks daily attendance records, grade history, and multi-year student trajectories. However, these survey dimensions---mother tongue, poverty, geography---are the ones most relevant to studying equity disparities.

We define dropout as a binary outcome: a school-age child enrolled in the previous academic year but not currently attending, following MINEDU's operational definition \cite{minedu2023alerta,inei2023enaho}. The 2020 wave (COVID-19 phone interviews) has a reduced sample of about 13,755 observations with 52\% null attendance records, which we dropped.
...
The sample is mostly Spanish-speaking (84\%), with Quechua, Aymara, and other indigenous languages making up the rest. About 65\% of observations are urban. The sample covers all three geographic regions: Costa (coast), Sierra (highlands), and Selva (Amazon lowlands).
...
Table~\ref{tab:language} shows large disparities: Otros ind\'{i}genas (0.219) and Awajun (0.205) have dropout rates 34\% higher than Castellano (0.153). For the fairness analysis, Ashaninka and Awajun are grouped under ``Otros ind\'{i}genas'' and 43 Extranjero students are excluded, accounting for the difference between $n=25{,}635$ (test set) and $n=25{,}592$ (Table~\ref{tab:fairness_language}).
...
Sierra and Selva have higher dropout rates than Costa, with a largely monotonic poverty gradient (Table~\ref{tab:region_poverty}). The interaction of language and rurality produces disparities larger than either dimension alone (Figure~\ref{fig:dropout_heatmap}, Appendix~\ref{sec:appendix_figures}).
...
The methodology has four stages: feature engineering, model training with temporal splitting, probability calibration, and fairness evaluation with SHAP interpretability.

\subsection{Feature Engineering}

We engineer 25 features in three categories: \emph{individual demographics} (8: age, sex, nationality, mother tongue dummies), \emph{household characteristics} (8: parent education, poverty index, poverty quintile, working status, household size, birthplace match), and \emph{district-level spatial indicators} (9: nightlight intensity z-score, census literacy and population z-scores, administrative completion rates, historical dropout rate). Table~\ref{tab:lr_coefficients} (Appendix~\ref{sec:appendix_tables}) lists all 25 features with their logistic regression coefficients. Nightlight z-score nulls (4.1\%) are imputed with 0.0 \cite{macnell2023implementing}; poverty quintiles use FACTOR07-weighted quantiles. As a robustness check, we extend to 31 features by adding overage-for-grade (years above expected age for enrollment level), an overage binary indicator, and four interaction terms (age $\times$ working status, age $\times$ poverty index, rural $\times$ parental education, secondary-age $\times$ household income). These are motivated by education literature on age-grade distortion as a dropout precursor and the compounding effect of simultaneous risk factors.

\subsection{Model Selection and Training}

We compare five model families: logistic regression (interpretable coefficients), LightGBM \cite{ke2017lightgbm} (primary model), XGBoost \cite{chen2016xgboost} (cross-architecture check), Random Forest (bagging baseline), and MLP (neural network baseline). If fairness findings hold across all five, they reflect data structure, not algorithmic artifacts.
...
We apply Platt scaling \cite{platt1999probabilistic} to the LightGBM model's raw outputs, reducing the validation Brier score from 0.186 to 0.116 (38\% improvement). The Platt parameters ($A=-6.236$, $B=4.443$) compress the probability range to a maximum of about 0.43.

\subsection{Fairness Evaluation Framework}

We use fairlearn \cite{bird2020fairlearn} to compute disaggregated metrics across seven demographic dimensions and three intersections, operationalizing the ``slice analysis'' of \citeN{baker2022algorithmic}. For each subgroup we report FNR, FPR, precision, and PR-AUC with survey weights. We privilege FNR as the primary fairness metric because FNR disparities indicate which populations the EWS misses. We complement FNR with FPR to capture the surveillance--invisibility trade-off predicted by the impossibility theorem of \citeN{chouldechova2017fair}. SHAP TreeExplainer \cite{lundberg2020local} decomposes predictions into feature-level contributions on the raw LightGBM model; interaction values use a 1,000-row test subsample.

To verify that FNR disparities are not artifacts of threshold selection, we sweep classification thresholds from 0.05 to 0.40 (the operationally meaningful range; calibrated probabilities reach a maximum of 0.48). Castellano speakers have the highest FNR at all thresholds from 0.05 through 0.20, including the optimal threshold of 0.185 (Table~\ref{tab:threshold_sweep}, Appendix~\ref{sec:appendix_tables}). At higher thresholds (0.25--0.40), where the compressed probability ceiling leaves little discriminative range, otros ind\'{i}genas overtakes castellano---but all groups exceed 0.70 FNR in this range.\footnote{Aimara ($n=76$) shows rank instability at extreme thresholds due to small sample size.}
...
\begin{table}[htbp]
  \caption{Model Performance Comparison Across Five Families (Survey-Weighted Metrics). The 31f column shows validation PR-AUC after adding 6 overage and interaction features.}\vspace*{1ex}
  \label{tab:models}
...
Indigenous language variables dominate the linear model (other-indigenous OR=2.20; Table~\ref{tab:lr_coefficients}, Appendix~\ref{sec:appendix_tables}). There is zero overlap in top-5 features between linear and tree-based models: feature ``importance'' depends on model family.

\subsection{Feature Robustness}

Expanding the feature set from 25 to 31 variables---adding overage-for-grade and interaction features---improves all five models' validation PR-AUC by 9--14\% (Table~\ref{tab:prauc_comparison}, Appendix~\ref{sec:appendix_tables}). The v2 LightGBM achieves validation PR-AUC of 0.291 and test PR-AUC of 0.257. Despite improved prediction, the FNR rank order across language groups is identical in both feature sets and all five model families (Table~\ref{tab:crossmodel_fnr}): castellano speakers remain the most-missed group in all ten model instances. This is the ``persist'' scenario: castellano FNR increases slightly from 0.633 to 0.663 (+4.7\%), demonstrating that the disparity is structural---better features improve prediction overall but do not resolve the castellano invisibility pattern.
...
Language dominates the fairness picture: region shows FNR variation driven by spatial profiles, poverty quintiles show a monotonic flagging gradient that partially tracks base rates, and the sex gap is minimal (FNR difference of 0.026). Nationality ($n=27$ non-Peruvian) is unusable for inference.
...
Table~\ref{tab:intersection} presents the intersection-level analysis (see also Figure~\ref{fig:fnr_heatmap} in Appendix~\ref{sec:appendix_figures}). Urban indigenous students face an FNR of 0.753---the model misses three out of four of their dropouts. This intersection group is invisible in both language-only analysis (where other-indigenous FNR is 0.22, driven by rural indigenous students) and geography-only analysis (where urban FNR is moderate). Only by crossing language and urbanicity does this disparity emerge, as \citeN{crenshaw1989demarginalizing} argued and \citeN{buolamwini2018gender} demonstrated computationally.
...
Table~\ref{tab:shap} and Figure~\ref{fig:shap_bar} show how the model makes predictions, directly addressing RQ2 (see also Figure~\ref{fig:shap_beeswarm} in Appendix~\ref{sec:appendix_figures} for the full feature-value distribution). The top five SHAP features are all spatial-structural variables: age, nightlight z-score, working status, census literacy z-score, and poverty index z-score. Identity features contribute little. Sex (es\_mujer) ranks 16th with a mean absolute SHAP value of 0.003, and nationality (es\_peruano) ranks last with effectively zero contribution, consistent with only $n=27$ non-Peruvian students in the sample.

The top 5 SHAP features have zero overlap with the top 5 logistic regression features (dominated by indigenous language dummies). Logistic regression assigns large coefficients to identity features; LightGBM achieves similar discrimination through correlated spatial-structural features. The model encodes structural inequities without using identity features directly. Removing protected attributes would not mitigate the documented disparities.
...
SHAP analysis shows that nightlight intensity, district-level dropout rates, and census literacy rates encode the spatial concentration of disadvantage. This creates blind spots for populations that do not match spatial stereotypes. Urban indigenous students are the clearest case: they live in areas with favorable spatial indicators but face educational barriers comparable to their rural counterparts. \citeN{perdomo2023difficult} showed that structural features predict dropout well; we show that reliance on such features creates predictable fairness failures at demographic intersections.

This spatial-proxy mechanism is not Peru-specific. Any EWS using school-level attendance rates, census poverty indicators, or geographic aggregates will face a similar pattern wherever spatial disadvantage correlates with demographic identity. The model's accuracy concentrates among students whose circumstances match their district's statistical profile and misses those whose disadvantage is not spatially encoded. When geographic predictors correlate with group membership, the model under-flags majority-group students in favorable-looking districts even when individual barriers persist, because spatial aggregates smooth over within-district demographic variation. The surveillance--invisibility axis is therefore a consequence of how these systems are built, not a Peru-specific data artifact. Practitioners should audit for spatial-proxy reliance before deployment, checking whether FNR disparities track the correlation between geographic features and demographic group membership.
...
Expanding the feature set from 25 to 31 variables---adding overage-for-grade and interaction features motivated by education dropout literature---improves all five models' PR-AUC by 9--14\% but slightly \emph{increases} castellano FNR from 0.633 to 0.663. Better prediction did not reduce the FNR gap; it marginally widened it. This confirms the castellano invisibility pattern is structural, not a feature-engineering artifact.

Group-specific threshold adjustment could reduce castellano invisibility, and supplementary identification mechanisms could address the urban indigenous blind spot. Both approaches redistribute rather than eliminate errors, and their implementation requires weighing operational costs against community consent \cite{mcmahon2020reenvisioning}.
...
Second, ENAHO's mother tongue variable (P300) is self-reported. Bilingual speakers may report Spanish as their mother tongue, which would undercount indigenous-language prevalence and understate the disparities we document.
...
Fifth, ENAHO's rotating panel design limits longitudinal analysis: only 22.0\% of school-age observations link across consecutive waves (18.9\% effective rate after quality filtering), too few for reliable trajectory features like income changes or sibling dropout events. This constrains us to cross-sectional variables, a limitation that administrative data would resolve.

Sixth, formal statistical guarantees for survey-weighted gradient boosting under complex designs remain an active research area \cite{macnell2023implementing}.

% =============================================================================
\section{Ethical Considerations}
% =============================================================================

We address positionality and data ethics relevant to this audit.
...
\paragraph{Data Ethics} This study uses publicly available, de-identified survey data (ENAHO) released by INEI for research purposes. No individual students can be identified, and no human subjects interaction was involved.

% =============================================================================
\section{Conclusion}
% =============================================================================

This proxy equity audit of survey-derived dropout risk in Peru ($N=150{,}135$; 2018--2023) identifies Spanish-speaking students---the demographic majority---as the group most systematically missed by the model (FNR = 0.633). Urban indigenous students may face even higher miss rates at the intersection of language and geography (pooled FNR = 0.69, $n=167$), though sample size limits precision. These findings hold across five model families and persist when expanding from 25 to 31 features (Table~\ref{tab:prauc_comparison}), confirming they reflect data structure rather than feature artifacts.

The proxy audit methodology shows that independent algorithmic accountability is possible using only publicly available survey data, applicable wherever direct system access is unavailable. But models that predict through spatial-structural features create predictable blind spots at demographic intersections where geographic and social profiles diverge. Feature ablation confirms that spatial features drive the castellano invisibility pattern, and the full model's combination of spatial and individual features produces the worst FNR outcome for the majority group.

Our power analysis shows a methodological ceiling: survey data cannot produce statistically significant intersectional fairness results for subgroups with fewer than $\sim$6 positive observations per year. Opening SIAGIE administrative data would enable direct system evaluation and the statistical power that intersectional auditing requires.
...
\begin{table}[H]
  \caption{False Negative Rate by Language Group Across Five Model Families. Each language group shows v1 (25 features) and v2 (31 features) results. Rank order is identical across all ten model instances. Aimara group ($n=76$) shows instability (MLP FNR=0.830); cross-architecture consistency claim is scoped to the castellano vs.\ indigenous pattern.}\vspace*{1ex}
...
\begin{table}[H]
  \caption{PR-AUC Before and After Feature Expansion (Validation Set, 2022).}\vspace*{1ex}
  \label{tab:prauc_comparison}
  \input{tables/table_12_prauc_comparison.tex}
\end{table}

\begin{table}[H]
  \centering
  \caption{FNR Rank Order Across Classification Thresholds (v2, 31 features). Castellano speakers have the highest FNR (rank 1) at all operationally meaningful thresholds (0.05--0.20).}\vspace*{1ex}
