...
\begin{itemize}
  \item A proxy equity audit framework, replicable in any country with nationally representative household surveys, demonstrating that independent algorithmic accountability is achievable using only public data---implemented as an open-source pipeline.
  \item A survey-weighted fairness audit spanning seven demographic dimensions and three intersections, revealing a surveillance--invisibility axis: indigenous-language students are over-flagged while the majority of Spanish-speaking dropouts are missed---disparities invisible to single-axis analysis \cite{crenshaw1989demarginalizing,buolamwini2018gender}.
...
The features available in ENAHO differ substantially from SIAGIE: our proxy model lacks daily attendance records, grade history, and multi-year student trajectories. However, the survey dimensions available---mother tongue, poverty, geography---are precisely those needed to study equity disparities.
...
Table~\ref{tab:shap} and Figure~\ref{fig:shap_bar} reveal how the model makes predictions, directly addressing RQ2 (see also Figure~\ref{fig:shap_beeswarm} in Appendix~\ref{sec:appendix_figures} for the full feature-value distribution). The top five SHAP features---age, nightlight z-score, working status, census literacy z-score, and poverty index z-score---are all spatial-structural variables. Identity features contribute minimally: the sex indicator (es\_mujer) ranks 16th out of 25 features with a mean absolute SHAP value of only 0.003, and the nationality indicator (es\_peruano) ranks 25th with effectively zero contribution, consistent with the $n=27$ non-Peruvian sample producing no learnable signal.
...
This spatial-proxy mechanism is not Peru-specific. Any EWS that incorporates school-level attendance rates, census poverty indicators, or geographic aggregates as predictors will face an analogous pattern wherever spatial disadvantage and demographic identity are correlated: the model's predictive accuracy will concentrate among students whose individual circumstances match their district's statistical profile, systematically missing those whose disadvantage is not spatially encoded. When the geographic features that predict dropout also correlate with demographic group membership, the model will under-flag students from majority groups whose district context appears favorable even when individual-level barriers persist---a direct consequence of how spatial aggregates smooth over within-district demographic variation. The surveillance--invisibility axis we document---over-flagging of spatially disadvantaged minorities, under-flagging of the majority whose dropout risk is invisible to district-level indicators---is therefore an emergent property of the EWS design paradigm rather than a data artifact specific to Peru. Educational data mining researchers and EWS practitioners should audit for spatial-proxy reliance before deployment, verifying that group-level FNR disparities do not track the correlation structure between geographic features and demographic group membership.
...
A second robustness check reinforces this conclusion: expanding the feature set from 25 to 31 variables---adding overage-for-grade and interaction features motivated by education dropout literature---improves all five models' PR-AUC by 9--14\% but slightly \emph{increases} castellano FNR from 0.633 to 0.663. That better prediction does not reduce the FNR gap, and indeed marginally widens it, demonstrates that the castellano invisibility pattern is a structural consequence of spatial proxy reliance, not a feature-engineering artifact.
...
Fifth, ENAHO's rotating panel design limits longitudinal analysis: only 22.0\% of school-age observations could be linked across consecutive waves (18.9\% effective rate after quality filtering), insufficient for constructing reliable trajectory features such as income changes or sibling dropout events. This constrains our feature space to cross-sectional variables and confirms a methodological limitation of survey-based dropout modeling that administrative data would resolve.
...
We address positionality and data ethics considerations relevant to this audit.
...
This proxy equity audit of survey-derived dropout risk in Peru ($N=150{,}135$; 2018--2023) identifies Spanish-speaking students---the demographic majority---as the group most systematically missed by the model (FNR = 0.633), with suggestive evidence that urban indigenous students face even higher miss rates at the intersection of language and geography (pooled FNR = 0.69, $n=167$). These findings hold across five model families with different architectures and persist when expanding from 25 to 31 features with overage-for-grade and interaction terms (Table~\ref{tab:prauc_comparison}), confirming they reflect data structure rather than feature artifacts.
